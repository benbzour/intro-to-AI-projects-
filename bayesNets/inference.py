...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for elimVariable in eliminationOrder:
            ...
            #we want to eliminate so long as there are more than 1 unconditioned vars remaining
            if len(joinedFactor.unconditionedVariables()) >= 2:
                joinedFactor = eliminate(joinedFactor, elimVariable)
                currentFactorsList.append(joinedFactor)
