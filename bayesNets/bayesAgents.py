        ...
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                for h in gameState.getPossibleHouses():
                    temp_dist = util.manhattanDistance(obsPos,h)
                    if temp_dist <= dist:
                        House = h 
                        dist = temp_dist
                if House == bottomLeftPos:
                    theHouse = BOTTOM_LEFT_VAL
                elif House == topLeftPos:
                    theHouse = TOP_LEFT_VAL
                elif House == bottomRightPos:
                    theHouse = BOTTOM_RIGHT_VAL
                elif House == topRightPos:
                    theHouse = TOP_RIGHT_VAL

                      

                if theHouse != food and theHouse != ghost:
                    if color == NO_OBS_VAL:
                        obsFactor.setProbability(assignment, 1)
                    else:
                        obsFactor.setProbability(assignment, 0)
                elif theHouse == ghost and ghost != food:
                    ...
                    if color == RED_OBS_VAL:
                        ...
                        obsFactor.setProbability(assignment, 0)
                    # print('in Ghost')
                    # print(theHouse,ghost)
                else:
                        obsFactor.setProbability(assignment, 0)
                   
                # print(ghost,food,theHouse,House,bottomLeftPos,topLeftPos)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in factor.getAllPossibleAssignmentDicts():
        temp_mostLikelyProb = factor.getProbability(assignment)

        if temp_mostLikelyProb > mostLikelyProb:
            mostLikelyProb = temp_mostLikelyProb
            val = assignment
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        food_factor = inference.inferenceByVariableElimination(self.bayesNet, BOTH_TOP_VAL, evidence, eliminationOrder)
        # ghost_factor = inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
        for assignment in food_factor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                food_left = assignment
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                food_right = assignment

        leftExpectedValue = food_factor.getProbability(food_left) * WON_GAME_REWARD + food_factor.getProbability(food_right) * GHOST_COLLISION_REWARD
        rightExpectedValue = food_factor.getProbability(food_left) * GHOST_COLLISION_REWARD + food_factor.getProbability(food_right) * WON_GAME_REWARD
