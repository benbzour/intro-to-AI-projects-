...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        for p in node.get_parents():
            inputs.append(self.get_output(p))
        return inputs
    ...
    def add(self, node):
        ...
        self.outputs[node] = np.array(node.forward(self.get_inputs(node)))
        self.gradient[node] = np.zeros_like(self.outputs[node])
    ...
    def backprop(self):
        ...
        for n in reversed(self.nodes):
            updated_gradient = n.backward(self.get_inputs(n),self.gradient[n])
            for i in range(len(n.get_parents())):
                # print(self.gradient[n.get_parents()[i]])
                self.gradient[n.get_parents()[i]] += updated_gradient[i]
    ...
    def step(self, step_size):
        ...
        for var in self.variables:
            var.data = var.data - self.gradient[var] * step_size
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        a = inputs[0] 
        x = inputs[1]
        adim = np.ones(a.shape)
        xdim = np.ones(x.shape)
        g = np.array(gradient)
        aa = gradient*adim 
        xx = g.sum(axis = 0) 
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        b = np.where(inputs[0] >= 0,inputs[0],0)
        return b
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        # ab1 = (a-b)*g
        # ab2 = (a-b)*(-g)
        # ab1 = ab1.mean(axis=1) / len(a)
        # ab2 = ab2.mean(axis=1) / len(a)
        # ab1 = np.reshape(ab1, (-1, a.shape[0])).T
        # ab2 = np.reshape(ab2, (-1, a.shape[0])).T

        ab1 = gradient*(1.0/(a.shape[0]*a.shape[1])*(a-b))
        ab2 = gradient*(1.0/(a.shape[0]*a.shape[1])*(b-a))
