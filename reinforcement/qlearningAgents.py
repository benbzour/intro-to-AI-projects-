...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        qVal = 0.0
        for f in featVector:
          qVal +=  self.weights[f] * featVector[f]
        return qVal
    ...
    def update(self, state, action, nextState, reward):
        ...
        for a in actions:
          vals.append(self.getQValue(nextState,a))
        if vals == []: 
          Q_nextState = 0.0
        else:
          Q_nextState = max(vals)
        diff = (reward + self.discount * Q_nextState ) - self.getQValue(state,action)

        featVector = self.featExtractor.getFeatures(state,action)

        for f in featVector:
          self.weights[f] = self.weights[f] + self.alpha * diff * featVector[f]
